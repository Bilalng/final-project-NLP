...
data_clean_dir = os.path.join(base_dir, "..", "data", "cleaned-reduced-data-set")
tfidf_dir = os.path.join(base_dir, "..", "data", "tf-idf-score")

os.makedirs(plots_dir, exist_ok=True)

# === Verileri Yükle ===
print("Veriler yükleniyor...")
lemmatized = pd.read_csv(os.path.join(data_clean_dir, "corpus_lemmatized.csv"))
stemmed = pd.read_csv(os.path.join(data_clean_dir, "corpus_stemmed.csv"))

# === Sütun adlarını belirle ===
lemma_col = 'lemmatized_text' if 'lemmatized_text' in lemmatized.columns else lemmatized.columns[0]
stem_col = 'stemmed_text' if 'stemmed_text' in stemmed.columns else stemmed.columns[0]

# === Giriş Metni Belirle ===
# Manuel giriş metni
input_text_lemma = "engin"
input_text_stem = "engin"
...
def tfidf_similarity(tfidf_model_csv_path, df, label, text_col, input_text):
    try:
        ...
        if not input_indices:
            ...
            input_indices = df_clean.index[df_clean[text_col].str.contains(f'\\b{input_text}\\b', case=False, na=False, regex=True)].tolist()
        ...
        for i, idx in enumerate(top5_idx):
            ...
            score = sims[idx]
            print(f"{i+1}. Skor: {score:.4f} | Metin: {text}")
...
tfidf_lemma_indices, tfidf_lemma_scores, tfidf_lemma_texts = tfidf_similarity(
    os.path.join(tfidf_dir, "tfidf_lemmatized.csv"), 
...
tfidf_stem_indices, tfidf_stem_scores, tfidf_stem_texts = tfidf_similarity(
    os.path.join(tfidf_dir, "tfidf_stemmed.csv"), 
...
for method, window, dim in configs:
    # Lemmatized model
    name_lemma = f"word2vec_lemma_{window}_{dim}_{method}"
    ...
    # Stemmed model
    name_stem = f"word2vec_stem_{window}_{dim}_{method}"
...
for model_name, (indices, scores, texts) in results.items():
    if texts:  # Eğer sonuç varsa
        input_text = input_text_lemma if 'lemma' in model_name else input_text_stem
...
plt.title("Jaccard Benzerlik Matrisi (18x18)", fontsize=16, fontweight='bold')
plt.xlabel("Modeller", fontsize=12)
plt.ylabel("Modeller", fontsize=12)
plt.xticks(rotation=90, fontsize=8)
plt.yticks(rotation=0, fontsize=8)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, "jaccard_matrix_18x18.png"), dpi=300, bbox_inches='tight')
plt.show()

print(f"\n[Jaccard benzerlik matrisi oluşturuldu: {plots_dir}/jaccard_matrix_18x18.png]")
...
print(f"   En başarılı model: {semantic_df_sorted.iloc[0]['Model']} (Ortalama: {semantic_df_sorted.iloc[0]['Ortalama']:.2f})")
print(f"   En düşük performans: {semantic_df_sorted.iloc[-1]['Model']} (Ortalama: {semantic_df_sorted.iloc[-1]['Ortalama']:.2f})")
...
print(f"   En benzer model çifti: {most_similar_pairs[0][0]} <-> {most_similar_pairs[0][1]} ({most_similar_pairs[0][2]:.3f})")
