...
data_processed_dir = os.path.join(base_dir, "..", "data","similar_words")
os.makedirs(plots_dir, exist_ok=True)
os.makedirs(data_processed_dir, exist_ok=True)

target_words = ["broke", "engine", "tire", "gearbox", "accident"]
...
for model_file in model_files:
    ...
    try:
        ...
        model_info = {
        'filename': model_file,
        'is_lemmatized': 'lemma' in model_file,  # isimde 'lemma' geçiyorsa lemmatized'dir
        'model_type': 'CBOW' if 'cbow' in model_file else 'Skip-gram',
        'window_size': int(model_file.split('win')[1].split('_')[0]),
        'vector_size': int(model_file.split('dim')[1].split('_')[0]),
...
# Benzer kelimeler tablosu oluşturma
